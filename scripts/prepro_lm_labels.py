"""
Some of their pre-trained model have different vocab than prepro_labels.py can offer
So we do this :)
"""
...
def process_caption(imgs, params):
    count_thr = params['word_count_threshold']
    ...
    # load in external vocab
    # vocab = json.load(open(params['vocab_json'], 'r'))
...
def main(params):
    imgs = json.load(open(params['dataset_json'], 'r'))
    imgs = imgs['images']
    ...
    out['ix_to_word'] = vocab # encode the (1-indexed) vocab
    out['images'] = []
    for i, img in enumerate(imgs):
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--dataset_json', required=True, help='input json file to process into hdf5')
    parser.add_argument('--vocab_json', required=True, help='we load in an external vocab file')
